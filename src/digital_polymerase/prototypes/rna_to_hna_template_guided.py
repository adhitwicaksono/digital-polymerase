#!/usr/bin/env python3
"""
Prototype 001B/001C.1: RNA to HNA Converter with Base-Attachment Correction

Digital Polymerase / XNA World Project

This script supports two HNA conversion modes:

1. full-template
   - For short RNAs where a full-length HNA template is available.
   - Uses the HNA template backbone/scaffold directly.
   - Transforms and transplants RNA bases onto the HNA scaffold.

2. chain-preserving
   - For RNA inputs longer than the HNA template.
   - Preserves source RNA chain-continuity atoms.
   - Introduces local HNA scaffold atoms from the HNA template:
     O4', C6', C1', C2' and selected local scaffold hydrogens if available.
   - Transforms RNA bases onto the newly placed HNA-like local scaffold.
   - This is a scalable HNA-like candidate generator, not a physically validated HNA model.

Default mode:
    auto

In auto mode:
    if HNA template length >= RNA length: use full-template
    else: use chain-preserving

Important:
- This is NOT a chemically or physically validated HNA modeling package.
- Outputs are computational candidate structures.
- They require visual inspection, topology/force-field review, energy minimization,
  and expert chemical validation before scientific interpretation.
"""
...
# Full-template HNA scaffold/backbone atom recognition.
HNA_BACKBONE_ATOMS = {
    "P", "OP1", "OP2", "O1P", "O2P",
    "O5'", "C5'", "C4'", "O4'", "C3'", "O3'",
    "C2'", "C1'", "C6'",
...
# In chain-preserving mode, these are introduced from the HNA template.
HNA_LOCAL_ATOMS = {
    "O4'", "C6'", "C1'", "C2'",
    "H1'", "1H2'", "2H2'", "1H6'", "2H6'",
    "H6'", "H2'", "H1'",
...
BACKBONE_ORDER = [
    ...
    "C6'", "C1'", "C2'",
    "C3'", "O3'",
    "1H6'", "2H6'", "H6'", "H1'", "1H2'", "2H2'", "H2'",
]

FULL_TEMPLATE_ANCHORS = ["C1'", "C2'", "C4'"]
CHAIN_TEMPLATE_ANCHORS = ["C3'", "C4'", "C5'", "O5'", "O3'"]
BASE_TRANSPLANT_ANCHORS = ["C1'", "C2'", "C4'"]
...
def normalize_base(res_name: str) -> str:
    ...
    # HNA examples: 6HA, 6HT, 6HG, 6HC.
    if "6HA" in r or r.endswith("A"):
        return "A"
    if "6HT" in r or r.endswith("T"):
        return "U"
    if "6HU" in r or r.endswith("U"):
        return "U"
    if "6HG" in r or r.endswith("G"):
        return "G"
    if "6HC" in r or r.endswith("C"):
        return "C"

    if r.startswith("A"):
        return "A"
    if r.startswith("U") or r.startswith("T"):
        return "U"
    if r.startswith("G"):
        return "G"
    if r.startswith("C"):
...
def is_backbone_atom(atom_name: str) -> bool:
    if atom_name in HNA_BACKBONE_ATOMS:
        return True
    if "'" in atom_name:
        return True
    if atom_name in {"H7", "H8", "H9", "H10", "H11", "H12", "H13", "H14"}:
        return True
    return False
...
def target_hna_residue_name(source_base: str, base_label_policy: str = "preserve") -> str:
    b = source_base.upper()
    if base_label_policy == "preserve":
        return {"A": "6HA", "U": "6HU", "G": "6HG", "C": "6HC"}.get(b, "6HN")
    if base_label_policy == "dna-like":
        return {"A": "6HA", "U": "6HT", "G": "6HG", "C": "6HC"}.get(b, "6HN")
...
def template_glycosidic_distance(template_residue: Dict[str, Atom], source_base: str) -> Optional[float]:
    """Return C1' to glycosidic atom distance from the selected HNA template residue."""
...
def adjust_base_attachment(
    ...
    """
    Translate all base atoms along the C1'->glycosidic-N vector so that the
    C1' to glycosidic atom distance matches the selected HNA template.

    This is a conservative post-alignment correction for chain-preserving HNA mode.
    It preserves base orientation and internal base geometry while correcting
    sugar-base attachment distance.
    """
    gly_atom = glycosidic_atom_for_base(source_base)
    if desired_distance is None:
        desired_distance = 1.38
...
def build_template_index(template: Dict[ResidueKey, Dict[str, Atom]]) -> Dict[str, List[Tuple[ResidueKey, Dict[str, Atom]]]]:
    ...
    for key in sorted_residue_keys(template):
        res = template[key]
        # skip residue fragments with too few atoms
        if len(res) < 6:
            continue
...
def select_template_for_base(
    ...
    if base in template_index and template_index[base]:
        ...
        return key, res, base, "exact base-class HNA template"

    # RNA U can use T-like HNA geometry if normalized as U from 6HT.
    if base == "U" and "T" in template_index and template_index["T"]:
        ...
        key, res = pool[i % len(pool)]
        return key, res, "T", "T-like HNA geometry donor used for RNA U"
    ...
    if base == "A" and "G" in template_index:
        ...
        key, res = pool[i % len(pool)]
        return key, res, "G", "purine fallback: A uses G-like HNA template"
    ...
    if base == "G" and "A" in template_index:
        ...
        key, res = pool[i % len(pool)]
        return key, res, "A", "purine fallback: G uses A-like HNA template"
    ...
    if base == "C" and "U" in template_index:
        ...
        key, res = pool[i % len(pool)]
        return key, res, "U", "pyrimidine fallback: C uses U/T-like HNA template"
    ...
    if base == "U" and "C" in template_index:
        ...
        key, res = pool[i % len(pool)]
        return key, res, "C", "pyrimidine fallback: U uses C-like HNA template"
    ...
    key, res = pool[i % len(pool)]
    return key, res, first, f"last-resort fallback: uses {first}-like HNA template"
...
def convert_full_template(
    ...
    *,
    ...
    if len(template_keys) < len(source_keys):
        raise ValueError(
            f"HNA template has fewer residues ({len(template_keys)}) than source RNA ({len(source_keys)}). "
    ...
    for idx, skey in enumerate(source_keys):
        ...
        records.append(
            ConversionRecord(
                ...
                note="full HNA scaffold from corresponding template residue; transformed source RNA base",
            )
        )
...
def convert_chain_preserving(
    ...
    *,
    ...
    if not template_index:
        raise ValueError("Could not build usable HNA template index")
    ...
    for skey in sorted_residue_keys(source):
        ...
        if len(used_chain_anchors) < 3:
            # Fallback to local sugar anchors if needed.
        ...
        # Preserve RNA chain-continuity atoms.
        ...
        # Insert local HNA scaffold atoms transformed into RNA chain frame.
        for atom_name, atom in tres.items():
            if atom_name in HNA_LOCAL_ATOMS:
        ...
        # Ensure required local atoms exist.
        required = ["C1'", "C2'", "C3'", "C4'"]
        missing = [a for a in required if a not in out_res]
        if missing:
            raise ValueError(f"Converted residue {skey} missing required local atoms: {missing}")

        # Transform RNA base atoms from source local frame onto the new HNA-like local frame.
        ...
        records.append(
            ConversionRecord(
                ...
                glycosidic_distance=gly,
                note=selection_note + "; RNA chain atoms preserved; HNA local scaffold inserted; RNA base transformed",
            )
        )
...
def write_pdb(
    ...
    *,
    ...
    with output_path.open("w", encoding="utf-8") as f:
        f.write("REMARK   Digital Polymerase Prototype 001B/001C.1\n")
        f.write("REMARK   RNA to HNA candidate reconstruction\n")
        f.write(f"REMARK   Mode: {mode}\n")
        f.write(f"REMARK   Source RNA sequence: {source_sequence}\n")
        f.write(f"REMARK   HNA template base-class sequence: {template_sequence}\n")
        f.write(f"REMARK   Base label policy: {base_label_policy}\n")
        if mode == "full-template":
            f.write("REMARK   Method: HNA scaffold from full template; RNA bases transformed and transplanted\n")
        else:
            f.write("REMARK   Method: RNA chain atoms preserved; HNA local scaffold inserted; RNA bases transformed\n")
        ...
        f.write("REMARK   WARNING: HNA residue names may be provisional, especially 6HU if preserve policy is used\n")
...
def validate_glycosidic(residues):
    ...
    for k in sorted_residue_keys(residues):
        ...
        status = "PASS" if 1.30 <= d <= 1.70 else "FAIL"
        records.append(DistanceRecord("C1'→glycosidic N", k[0], k[1], k[2], "C1'", k[1], k[2], atom_b, d, 1.30, 1.70, status))
    return summarize_distances("C1'→glycosidic N", records, missing, 1.30, 1.70)
...
def build_validations(source, template, output, *, skip_numbering_gaps=True):
    def common(residues):
        return {
            ...
            "P(i)→O5'(i)": validate_intra(residues, "P", "O5'", 1.40, 1.80, "P(i)→O5'(i)"),
            "C1'→glycosidic N": validate_glycosidic(residues),
            "C1'→C2'": validate_intra(residues, "C1'", "C2'", 1.35, 1.90, "C1'→C2'"),
            "C2'→C3'": validate_intra(residues, "C2'", "C3'", 1.35, 1.85, "C2'→C3'"),
        }

    output_checks = common(output)
    output_checks.update({
        "C1'→C6'": validate_intra(output, "C1'", "C6'", 1.35, 1.85, "C1'→C6'"),
        "C6'→O4'": validate_intra(output, "C6'", "O4'", 1.20, 1.60, "C6'→O4'"),
        "C4'→O4'": validate_intra(output, "C4'", "O4'", 1.20, 1.70, "C4'→O4'"),
    })

    template_checks = common(template)
    template_checks.update({
        "C1'→C6'": validate_intra(template, "C1'", "C6'", 1.35, 1.85, "C1'→C6'"),
        "C6'→O4'": validate_intra(template, "C6'", "O4'", 1.20, 1.60, "C6'→O4'"),
        "C4'→O4'": validate_intra(template, "C4'", "O4'", 1.20, 1.70, "C4'→O4'"),
    })

    return {
        "source RNA": common(source),
        "HNA template": template_checks,
...
def write_report(
    ...
    *,
    ...
    c1c6 = validations["output"]["C1'→C6'"]
    c6o4 = validations["output"]["C6'→O4'"]
    ...
    report = f"""# RNA → HNA Conversion Report
...
**HNA local scaffold status:** `C1′→C6′ {validation_status(c1c6)}`, `C6′→O4′ {validation_status(c6o4)}`
...
- HNA template PDB: `{template_path}`
...
- HNA template base-class sequence: `{tmpl_seq}`
...
- Chain-preserving HNA-template anchors: `{", ".join(CHAIN_TEMPLATE_ANCHORS)}`
...
{failed_table(validations["output"]["C1'→C6'"], n=20)}

### Worst output HNA C6′–O4′ links

{failed_table(validations["output"]["C6'→O4'"], n=20)}
...
def convert(
    ...
    *,
    ...
    if not template:
        raise ValueError(f"No residues parsed from HNA template PDB: {template_path}")
    ...
    print("=" * 80)
    print("Digital Polymerase Prototype 001B/001C.1")
    print("RNA → HNA candidate reconstruction")
    print("=" * 80)
    print(f"Source RNA: {source_path}")
    print(f"HNA template: {template_path}")
    ...
    print(f"C1′→C6′: {validation_status(c1c6)} "
          f"(measured={c1c6.measured}, failed={c1c6.failed}, mean={c1c6.mean:.3f} Å)")
    print(f"C6′→O4′: {validation_status(c6o4)} "
          f"(measured={c6o4.measured}, failed={c6o4.failed}, mean={c6o4.mean:.3f} Å)")
    print("=" * 80)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Prototype 001B/001C.1: RNA to HNA converter.")
    parser.add_argument("--rna", "--source", dest="rna", required=True, type=Path, help="Source RNA PDB file")
    parser.add_argument("--template", required=True, type=Path, help="HNA template PDB file")
    parser.add_argument("--output", required=True, type=Path, help="Output HNA-like candidate PDB")
